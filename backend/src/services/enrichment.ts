...
export async function enrich(job: Job): Promise<Job> {
  if (!process.env.DEEPSEEK_API_KEY) return job;
  if (process.env.AI_PUBLIC_JOB_ENRICHMENT !== 'true') return job;
  ...
  const hash = Array.from(
    new Uint8Array(
      await crypto.subtle.digest(
        ...
        new TextEncoder().encode(
          `${process.env.DEEPSEEK_MODEL || 'deepseek-chat'}:${source}`,
        ),
      ),
    ),
  )
    ...
    .join('');
  try {
    const { rows } = await query<{ summary: string; tags: Interest[] }>(
      'select summary,tags from job_enrichment where hash=$1',
      [hash],
    );
    if (rows[0]) return { ...job, ...rows[0] };
    await limit('ai-global-hour', 30, 3600);
    await limit('ai-global-day', 150, 86400);
