...
export async function api<T>(path:string,method='GET',data?:unknown,retry=true):Promise<T>{
 ...
 let result:{error?:string};try{result=await r.json();}catch{throw new Error('Le service ne répond pas. Réessaie dans un instant.');}if(!r.ok)throw new Error(result.error||'Une erreur est survenue.');return result as T;
}
