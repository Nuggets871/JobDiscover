...
export async function authAction(req:Request,c:Config,action:string,input:unknown){
 ...
 if(['login','signup','recover'].includes(action)){
  ...
  const data=await r.json() as {access_token:string;refresh_token:string;user:{email_confirmed_at?:string}};if(!data.user.email_confirmed_at)throw new AppError(401,'Confirme ton adresse e-mail.');return sessionCookies(json({ok:true}),c,data);
 }
 if(action==='confirm'){
  const hash=string(v.token_hash,200);const type=string(v.type,20);if(!/^[a-zA-Z0-9_-]{20,200}$/.test(hash)||!['email','signup','recovery'].includes(type))throw new AppError(400,'Lien invalide.');await limit(c,'auth-confirm',60,60);
  const r=await authFetch(c,'verify',{token_hash:hash,type});if(!r.ok)throw new AppError(400,'Ce lien a expiré ou a déjà été utilisé. Demande un nouvel e-mail.');return sessionCookies(json({ok:true,recovery:type==='recovery'}),c,await r.json());
 }
 if(action==='refresh'){const token=refreshCookie(req,c);if(!token)throw new AppError(401,'Connecte-toi pour continuer.');const r=await authFetch(c,'token?grant_type=refresh_token',{refresh_token:token});if(!r.ok)return sessionCookies(json({error:'Ta session a expiré.'},401),c);return sessionCookies(json({ok:true}),c,await r.json());}
 if(action==='logout'){try{const user=await authenticate(req,c);await authFetch(c,'logout?scope=global',undefined,user.token);}catch{}return sessionCookies(json({ok:true}),c);}
 const user=await authenticate(req,c);await limit(c,`account:${user.id}`,10,600);
