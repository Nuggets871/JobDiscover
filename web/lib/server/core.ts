...
export async function config(): Promise<Config> {
  ...
  try {
    ...
    bindings = runtime.env as unknown as Config;
  } catch {}
...
export function assertOrigin(req: Request, c: Config) {
  if (
    !c.APP_ORIGIN ||
    req.headers.get('origin') !== new URL(c.APP_ORIGIN).origin
  )
    throw new AppError(403, 'Requête non autorisée.');
  if (req.headers.get('sec-fetch-site') === 'cross-site')
    throw new AppError(403, 'Requête non autorisée.');
}
export async function body(req: Request) {
  if (!req.headers.get('content-type')?.startsWith('application/json'))
    throw new AppError(415, 'Format attendu : JSON.');
  const length = Number(req.headers.get('content-length') ?? 0);
  if (length > 16384) throw new AppError(413, 'Requête trop volumineuse.');
  ...
  while (true) {
    ...
    if (size > 16384) {
      await reader.cancel();
      throw new AppError(413, 'Requête trop volumineuse.');
    }
    chunks.push(value);
  }
...
function cookieName(c: Config, name: string) {
  return c.APP_ORIGIN?.startsWith('https:')
    ? `__Host-jd_${name}`
    : `jd_${name}`;
}
export function sessionCookies(
  ...
  const secure = c.APP_ORIGIN?.startsWith('https:') ? '; Secure' : '';
  for (const [name, value, age] of [
    [
      'access',
      session?.access_token || '',
      session ? Math.min(session.expires_in || 3600, 3600) : 0,
    ],
    ['refresh', session?.refresh_token || '', session ? 60 * 60 * 24 * 7 : 0],
  ] as const) {
    r.headers.append(
      'Set-Cookie',
      `${cookieName(c, name)}=${value}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${age}${secure}`,
    );
  }
  return r;
}
export type Session = { id: string; email: string; token: string };
export async function authenticate(req: Request, c: Config): Promise<Session> {
  if (!configured(c))
    throw new AppError(
      503,
      'Les comptes seront disponibles après la configuration du service. Tu peux explorer la démonstration.',
    );
